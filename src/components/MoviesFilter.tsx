...
export default function MoviesFilter({ movies }: { movies: Movie[] }) {
  ...
  return (
    <div className="max-w-10/12 mx-auto px-4">
      ...
      {/* Movies Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 p-4 justify-items-end">
        {filtered.map(movie => (
          <Link
            key={movie.id}
          ...
          >
            <div className="relative bg-white/5 rounded-lg border-[0.5px] hover:border-[0.1px] border-gray-800 hover:border-violet-500 overflow-hidden shadow-sm hover:scale-105 transform transition duration-300">

              {/* Image */}
              <div className="relative w-full h-56 sm:h-64 md:h-56 lg:h-100 overflow-hidden">
                <img
                ...
                <div className="absolute inset-0 hidden group-hover:flex flex-col items-center justify-center text-center bg-black/50 backdrop-brightness-80 px-2">

                  {/* Title */}
                  <p className="text-sm font-semibold text-white max-w-2xl">
                    {movie.title}
                  </p>
