...
export default function EditMovieForm({ id, movie }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const body = {
      ...
      is_18_plus: fd.get("is_18_plus") === "on",
    };
  ...
  return (
    <div className="max-w-xl mx-auto bg-white/80 p-8 rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <label className="flex items-center gap-2 p-2 border rounded bg-red-50 cursor-pointer">
          <input type="checkbox" name="is_18_plus" defaultChecked={movie?.is_18_plus ?? false} className="w-4 h-4" />
          <span className="text-sm font-medium text-red-600">Mark as +18 content</span>
        </label>
