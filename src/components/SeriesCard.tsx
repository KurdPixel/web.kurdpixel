    ...
    <Link href={`/series/${series.slug}`} className="block group">
      <div className="relative bg-white/5 rounded-lg border-[0.5px] hover:border-[0.1px] border-gray-800 hover:border-violet-500 overflow-hidden shadow-sm hover:scale-105 transform transition duration-200">
        ...
        {series.total_seasons && (
          <div className="absolute top-2 left-2 z-10 bg-black/30 backdrop-blur-lg border border-white/10 text-white font-bold px-2 py-1 rounded-xl text-xs">
            {series.total_seasons} فصل
          </div>
        )}
