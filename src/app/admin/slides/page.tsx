    ...
    <main className="min-h-screen pt-28 pb-8 px-4 bg-[#0f0f0f]">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Manage Carousel Slides</h1>
          <p className="text-white/60">Add or remove slides for the main page carousel</p>
        </div>

        {/* Add Slide Form */}
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg p-6 mb-8">
          <h2 className="text-xl font-semibold text-white mb-4">Add New Slide</h2>
          <form onSubmit={handleAddSlide} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-white/80 mb-2">Movie Slug (Watch Button)</label>
              <input
                type="text"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 disabled:bg-violet-600/50 text-white font-medium transition-colors"
            >
        ...
        {/* Slides List */}
        <div>
          <h2 className="text-xl font-semibold text-white mb-4">Current Slides ({slides.length})</h2>

          {loading ? (
            <div className="text-white/60">Loading slides...</div>
          ) : slides.length === 0 ? (
            <div className="text-white/60">No slides yet. Add one above!</div>
          ) : (
            <div className="grid gap-4">
              {slides.map((slide, idx) => (
                <div key={slide.id} className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg p-4 flex gap-4">
                  {/* Thumbnail */}
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="text-white font-medium mb-1">Slide #{idx + 1}</h3>
                      {slide.title && <p className="text-violet-300 text-sm font-semibold mb-1">{slide.title}</p>}
                      {slide.description && <p className="text-white/70 text-sm mb-2">{slide.description}</p>}
                      <p className="text-white/60 text-sm break-all">{slide.image_url}</p>
